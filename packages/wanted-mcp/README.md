...
Wanted의 공개 채용 검색 화면을 사용자가 볼 수 있는 브라우저에서 열고 현재 화면에 표시된 공고를 최대 20건 읽는 독립 TypeMCP stdio 서버입니다. 채용 API와 인증키를 사용하지 않습니다.
...
https://www.wanted.co.kr/search에서 공개 검색을 한 번 실행합니다.
...
검색어에는 query와 includeKeywords만 넣습니다. locations는 공개 지역 팝업에서 복수 선택합니다. 경력 슬라이더는 접근 가능한 입력이 없어 좌표 드래그로 조작하지 않으며, 고용형태와 근무방식도 현재 공개 검색 화면에서 지원하지 않습니다. 미적용 조건은 filters.skipped로 반환합니다. 이력서 원문이나 개인정보를 입력하지 마세요.

## 화면 추출

- Playwright Core로 설치된 Chrome을 headless: false로 실행합니다.
- 새 비영구 컨텍스트와 탭 하나를 사용합니다.
- a[href*="/wd/"]에 해당하고 화면에 실제 크기가 있는 HTTPS 링크만 후보로 봅니다.
...
사용 전 루트 COMPLIANCE.md와 Wanted 최신 약관을 확인하세요.
