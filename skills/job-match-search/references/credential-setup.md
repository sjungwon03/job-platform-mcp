...
아직 승인되지 않은 플랫폼은 신청 링크를 안내하고 나중에 설정할 수 있게 한다. 승인 없이 진행하겠다고 하면 연결된 플랫폼, 사용자가 직접 제공한 공고 또는 개인용 브라우저 검색 모드를 안내한다. 브라우저 모드는 개인·비상업용 확인과 책임 고지 수락 후 [개인용 브라우저 검색](browser-search.md)에 따라 사용한다.
...
공식 승인을 받지 못했거나 API 할당량을 소진한 경우에도 플랫폼 웹사이트, 모바일 내부 API 또는 검색 페이지를 백그라운드에서 대량 수집하지 않는다. 로그인·CAPTCHA·robots.txt·IP 차단·속도 제한을 우회하지 않는다.

개인용 브라우저 검색은 사용자가 명시적으로 요청한 단일 검색을 화면이 보이는 브라우저에서 수행하고 현재 결과를 제한적으로 읽는 별도 모드다. 외부 서비스, 반복 모니터링 또는 데이터 축적에는 사용할 수 없으며, 그 경우 플랫폼의 범위가 명시된 서면 허가와 별도 법률 검토가 선행되어야 한다.
